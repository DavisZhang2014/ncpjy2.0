...
if(isset($_GET['id']) && $_GET['action'] == 'recover'){
	_query(" UPDATE tb_user SET del = '0' WHERE id = '{$_GET['id']}' ");
	if (_affected_rows() == 1)
	{
		_location('恭喜你，恢复成功！','member_delete_list.php');
	}
	else
	{
		_location('很遗憾，恢复失败！','member_delete_list.php');
	}
}
global $_pagesize,$_pagenum;

_page("SELECT id FROM tb_user WHERE del='1'",5);
$_result = _query("SELECT id,username,state,sex,email,reg_time FROM tb_user WHERE del='1' LIMIT $_pagenum,$_pagesize");
	...
	<div id="member_list">
		...
		<tr><th>ID</th><th>用户名</th><th>性别</th><th>邮箱</th><th>注册时间</th><th>操作</th></tr>
			...
			<tr>
				...
				<td>[<a href="?id=<?php echo $_html['id']?>&action=recover">恢复</a>]</td>
			</tr>
			<?php 
				}
