...
if($_GET['action'] == "modify")
{
	...
	if (!!$_rows = _fetch_array("SELECT uniqid FROM tb_user WHERE username='{$_COOKIE['username']}' AND password='{$_clean['password1']}' LIMIT 1"))
	{
	...
	if (_affected_rows() == 1)
	{
		_close();
		_session_destroy();
		_location('恭喜你，密码修改成功！','member_PSW_modify.php');
	}
	else
	{
		_close();
		_session_destroy();
		_location('很遗憾，密码修改失败！','member_PSW_modify.php');
	}
}
...
{
	//获取数据
	$_rows =_fetch_array("SELECT username,sex,email FROM tb_user WHERE username= '{$_COOKIE['username']}'");
	if($_rows)
	{
		$_html= array();
		$_html['question'] = $_rows['question'];
		$_html['answer']=$_rows['answer'];
		$_html = _html($_html);
